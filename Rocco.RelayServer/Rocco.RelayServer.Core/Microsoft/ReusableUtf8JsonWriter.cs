...
{
    [ThreadStatic] private static ReusableUtf8JsonWriter _cachedInstance;
    ...
    public static ReusableUtf8JsonWriter Get(IBufferWriter<byte> stream)
    {
        ...
        if (writer == null) writer = new ReusableUtf8JsonWriter(stream);

        // Taken off the thread static
        _cachedInstance = null;
#if DEBUG
        if (writer._inUse) throw new InvalidOperationException("The writer wasn't returned!");
